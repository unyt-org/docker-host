...
/***** imports */
import { Compiler, compiler_options, PrecompiledDXB, ProtocolDataTypesMap} from "../compiler/compiler.js"; // Compiler functions
import type { BlockchainAdapter } from "../network/blockchain_adapter.js";
import { DecimalRef, IntegerRef, Pointer, PointerProperty, Value, TextRef, BooleanRef} from "./pointers.js";
import { Endpoint, endpoints, LOCAL_ENDPOINT, Target, target_clause, WildcardTarget } from "../types/addressing.js";
import { RuntimePerformance } from "./performance_measure.js";
import { NetworkError, PermissionError, PointerError, RuntimeError, SecurityError, ValueError, Error as DatexError, CompilerError, TypeError, SyntaxError, AssertionError } from "../types/errors.js";
...
import { compile_info, datex_scope, dxb_header, routing_info } from "../utils/global_types.js";
import { Markdown } from "../types/markdown.js";
import { Type } from "../types/type.js";
import { Tuple } from "../types/tuple.js";
import { DatexObject } from "../types/object.js";
import { Crypto } from "../runtime/crypto.js";
import { ProtocolDataType } from "../compiler/protocol_types.js";
import { base64ToArrayBuffer, buffer2hex, fetch, fs, getFileContent, getLocalFileContent, getLocalFileTextContent } from "../utils/utils.js";
import { IOHandler } from "./io_handler.js";
import { DX_PERMISSIONS, DX_TYPE, DX_VALUE, INVALID, MAX_UINT_16, NOT_EXISTING, UNKNOWN_TYPE, VOID, WILDCARD } from "./constants.js";
import { baseURL, client_type, DEFAULT_HIDDEN_OBJECT_PROPERTIES, logger, NodeBuffer, TypedArray } from "../utils/global_values.js";
...
import { fundamental } from "../types/abstract_types.js";
import { IterationFunction as IteratorFunction, Iterator, RangeIterator } from "../types/iterator.js";
import { Assertion } from "../types/assertion.js";
import { $$, datex, eternal, f, pointer, static_pointer } from "../datex_short.js";
import { Maybe } from "../types/maybe.js";
import { Task } from "../types/task.js";
import { DATEX_ERROR } from "../types/error_codes.js";
import { cnf, Conjunction, Disjunction, Logical, Negation } from "../types/logic.js";
import { Logger } from "../utils/logger.js";
import { Decompiler } from "./decompiler.js";
import { Debugger } from "./debugger.js";
import { Datex, scope } from "../datex.js";

import "../types/native_types.js"; // load prototype overrides
import { Time } from "../types/time.js";

// @ts-ignore
export const ReadableStream = <typeof globalThis.ReadableStream>(client_type=="node" ? (await import('node:stream/web')).ReadableStream : globalThis.ReadableStream);
...
export class Runtime {
    ...
    static #saved_local_strings = new WeakMap<Object, Map<string, Datex.Pointer<string>>>();
    static #not_loaded_local_strings = new WeakMap<Object, Set<string>>();
    
    public static getLocalString(local_map:{[lang:string]:string}):Datex.Pointer<string> {
        return Datex.Pointer.createTransform([PointerProperty.get(Runtime.ENV, 'LANG')], 
        (lang:string) => local_map[lang], // sync js mapping
        `var lang = #env->LANG; always (${Runtime.valueToDatexString(local_map)}.(lang))` // used for persistent DATEX storage
        );
    }

    public static getLocalStringFromMap(local_map:{[lang:string]:{[key:string]:string}}, key:string):Datex.Pointer<string> {
        ...
        // create new transform pointer
            ...
            const value_available = (key in local_map[Runtime.ENV.LANG]);

            const string_transform = Datex.Pointer.createTransform([PointerProperty.get(Runtime.ENV, 'LANG')], 
            (lang:string) => this.transformLangValue(lang, local_map, key), // sync js mapping
            value_available ? `var lang = #env->LANG; always (${Runtime.valueToDatexString(local_map)}.(lang).${Runtime.valueToDatexString(key)})` : '' // used for persistent DATEX storage
            );
            // add to saved string
    ...
    public static newLocalStringsMapValues(local_map:{[lang:string]:{[key:string]:string}}) {
        ...
        for (let key of this.#not_loaded_local_strings.get(local_map)??[]) {
            if (lang in local_map && key in local_map[lang]) {
                local_strings_map.get(key).value = this.transformLangValue(lang, local_map, key);
                local_strings_map.get(key).setDatexTransform(`var lang = #env->LANG; always (${Runtime.valueToDatexString(local_map)}.(lang).${Runtime.valueToDatexString(key)})`) // used for persistent DATEX storage
    ...
    private static transformLangValue(lang:string, local_map:{[lang:string]:{[key:string]:string}}, key:string){
        return (lang in local_map ? local_map[lang]?.[key] : local_map['en']?.[key]) ?? '?UNKNOWN?'
    }
    ...
    public static async getURLContent(url_string:string, raw?:boolean, cached?:boolean): Promise<any>
    public static async getURLContent(url:URL, raw?:boolean, cached?:boolean): Promise<any>
    public static async getURLContent(url_string:string|URL, raw=false, cached = false) {
        ...
        if (url.protocol == "https:" || url.protocol == "http:") {
            ...
            if (type == "application/datex" || type == "text/dxb" || url_string.endsWith(".dxb")) {
                ...
                const content = await response.text();
                if (raw) result = [content, type];
                else result = await Datex.Runtime.datexOut([content, [], {sign:false, encrypt:false, type:ProtocolDataType.DATA}]);
            }
                ...
                const content = await response.arrayBuffer()
                if (raw) result = [content, type];
                else result = Type.get('std:image').cast(content);
            }
            ...
            if (filePath.endsWith('.dxb')) {
                const content = getLocalFileContent(url);
                if (raw) result = [content, "application/datex"];
                else result = await this.executeDXBLocally(content, url);
            }
            else if (filePath.endsWith('.dx')) {
                const content = getLocalFileTextContent(url);
            ...
            // TODO more file formats (images, ..)

            else  {
                if (raw) result = [getLocalFileTextContent(url), "text/plain"]
                else result = getLocalFileTextContent(url)
            }
     ...
     * @returns evaluated DATEX result
     */
    public static async decodeValueBase64(dxb_base64:string, outer_serialized=false):Promise<any> {
        // create scope
     ...
     * @returns evaluated DATEX result
     */
    public static async decodeValue(dxb:ArrayBuffer, outer_serialized=false):Promise<any> {
        // create scope
        const scope = Runtime.createNewInitialScope();
        scope.outer_serialized = outer_serialized;
        // set dxb as scope buffer
    ...
    public static async cloneValue<T>(value:T):Promise<T> {
        ...
        return await Runtime.decodeValue(Compiler.encodeValue(value, undefined, true, false, true));
    }
    ...
    public static async deepCloneValue<T>(value:T):Promise<T> {
        return await Runtime.decodeValue(Compiler.encodeValue(value, undefined, true, true, true));
    }
     ...
     * @returns evaluated DATEX result
     */
    public static async executeDatexLocally(datex:string|PrecompiledDXB, data?:any[], options?:compiler_options, context_location?:URL):Promise<any> {
    ...
    public static async init(endpoint?:Endpoint) {
        ...
        // std.sleep
        const sleep = DatexFunction.createFromJSFunction(async (time_ms:bigint)=>{
            return new Promise<void>(resolve=>setTimeout(()=>resolve(), Number(time_ms)));
        });

        // std.localtext
        const localtext = DatexFunction.createFromJSFunction(async (local_map:{[lang: string]: string})=>{
        ...
        // std.types 
        // TODO await Datex.Runtime.getURLContent('https://cdn.unyt.org/datex/type_info.dx')
        this.STD_TYPES_ABOUT =  await this.parseDatexData(await getFileContent("/unyt_core/dx_data/type_info.dx", './unyt_core/dx_data/type_info.dx'))
    ...
    public static parseHeaderSynchronousPart(dxb:ArrayBuffer):[dxb_header, Uint8Array, Uint8Array, number, Uint8Array, ArrayBuffer] {
        ...
        routing_info.sender = header.sender = Compiler.extractHeaderSender(header_uint8, last_index);
        i = last_index[0];
    ...
    private static _valueToDatexString(value:any, formatted = false, depth=0, collapse_pointers=false, deep_collapse = false, pointer_anchors?:[string,string], _serialized = false, parents = new Set<any>()): string {
        ...
        if (type && !type.is_primitive && type.is_complex && type != Type.std.Scope) string = type.toString() + (formatted ? " ":"") + string;
    ...
    = {
        ...
        async setInternalVarReference(SCOPE:datex_scope, name:number|string, reference:any, save_persistent = false) {
            ...
            if (reference === VOID) delete SCOPE.internal_vars[name];
                ...
                // otherwise, lose reference (copy value) TODO
                    ...
                    SCOPE.internal_vars[name] = await Runtime.cloneValue(reference);
                }
                ...
                try {
                    ...
                    switch (action_type) {
                        ...
                        case BinaryCode.ADD:
                            ...
                            else if (current_val instanceof TextRef && typeof value_prim == "string") current_val.value += value; // primitive pointer operations
                            else if (current_val instanceof DecimalRef && typeof value_prim == "number") current_val.value = current_val.value + value_prim;
                            else if (current_val instanceof IntegerRef && typeof value_prim == "bigint") current_val.value = current_val.value + value_prim;
                            else if (current_val instanceof DecimalRef && typeof value_prim == "bigint") current_val.value = current_val.value + Number(value_prim);
                        ...
                        case BinaryCode.SUBTRACT:
                            ...
                            else if (current_val instanceof DecimalRef && typeof value_prim == "number") current_val.value = current_val.value - value_prim; // primitive pointer operations
                            else if (current_val instanceof IntegerRef && typeof value_prim == "bigint") current_val.value = current_val.value - value_prim;
                            else if (current_val instanceof DecimalRef && typeof value_prim == "bigint") current_val.value = current_val.value - Number(value_prim);
                            else if (current_val instanceof IntegerRef && typeof value_prim == "number") throw new ValueError("Cannot apply a <decimal> value to an <integer> pointer", SCOPE);
                        ...
                        case BinaryCode.MULTIPLY:
                            if (current_val instanceof DecimalRef && typeof value_prim == "number") current_val.value = current_val.value * value_prim; // primitive pointer operations
                            else if (current_val instanceof IntegerRef && typeof value_prim == "bigint") current_val.value = current_val.value * value_prim;
                            else if (current_val instanceof DecimalRef && typeof value_prim == "bigint") current_val.value = current_val.value * Number(value_prim);
                            else if (current_val instanceof IntegerRef && typeof value_prim == "number") throw new ValueError("Cannot apply a <decimal> value to an <integer> pointer", SCOPE);
                            else if (typeof current_val_prim == "number" && typeof value_prim == "number") Runtime.runtime_actions.setProperty(SCOPE, parent, key, current_val_prim*value_prim) // subtract
                        ...
                        case BinaryCode.POWER:
                            if (current_val instanceof DecimalRef && typeof value_prim == "number") current_val.value = current_val.value ** value_prim; // primitive pointer operations
                                ...
                                else current_val.value = current_val.value ** value_prim;
                            }
                            else if (current_val instanceof DecimalRef && typeof value_prim == "bigint") current_val.value = current_val.value ** Number(value_prim);
                            else if (current_val instanceof IntegerRef && typeof value_prim == "number") throw new ValueError("Cannot apply a <decimal> value to an <integer> pointer", SCOPE);
                            else if (typeof current_val_prim == "number" && typeof value_prim == "number") Runtime.runtime_actions.setProperty(SCOPE, parent, key, current_val_prim*value_prim) // subtract
        ...
        async insertToScope(SCOPE:datex_scope, el:any, literal_value = false){
            ...
            else if (INNER_SCOPE.active_object && (INNER_SCOPE.waiting_collapse || 'waiting_key' in INNER_SCOPE)) {
            ...
            */
            ...
            // sync pointer
                ...
                if (!(pointer instanceof Value) || (pointer instanceof Pointer && pointer.is_anonymous)) throw new ValueError("sync expects a reference value", SCOPE);
                ...
                if (to instanceof Endpoint) {
                    ...
                    logger.success(SCOPE.sender + " subscribed to " + pointer.idString());
                    ...
                    if (pointer.is_origin) {
                        ...
                        if (!pointer.value) throw new PointerError("Pointer does not exist", SCOPE)
                        // valid, add subscriber
                        else {
                            pointer.addSubscriber(SCOPE.sender);
                            INNER_SCOPE.active_value = await Runtime.cloneValue(pointer.value);
                        }
                    }
                ...
                if (!compared) {
                    switch (INNER_SCOPE.compare_type) {
                        ...
                        case BinaryCode.LESS_EQUAL: {
                            let typeA = Type.ofValue(a);
                            let typeB = Type.ofValue(b);
                            ...
                            if (! ((typeA==Type.std.decimal || typeA==Type.std.integer) && (typeB == Type.std.decimal || typeB == Type.std.integer))) {
                                ...
                                let comp = typeA.handleCompare(a,b);
        ...
        while (true) {
            ...
            switch (token) {
                ...
                case BinaryCode.INIT_INTERNAL_VAR: { 
                    ...
                    let jmp_index = SCOPE.buffer_views.data_view.getUint32(SCOPE.current_index);
					SCOPE.current_index += Uint32Array.BYTES_PER_ELEMENT;

                    // already exists?
                    if (name in SCOPE.internal_vars) {
                        SCOPE.current_index = jmp_index; // jump to end of init block
                    }
                ...
                case BinaryCode.SCOPE_BLOCK: {  
                    ...
                    // REMOTE
                        ...
                        if (INNER_SCOPE.scope_block_vars.length) {
                            ...
                            let var_dxb = <ArrayBuffer> await Compiler.compile(variables_insert_code, INNER_SCOPE.scope_block_vars, undefined, false, false, false, undefined, Infinity)
                        ...
                        let sid = Compiler.generateSID();
                        let full_dxb = await Compiler.appendHeader(buffer, 
                            true,
                        ...
                        // datex out to filter
                        let res = await Runtime.datexOut(full_dxb, remote, sid, true, undefined, (scope, header, error)=>{
                ...
                case BinaryCode.DEFAULT: {
                    const jmp_index = SCOPE.buffer_views.data_view.getUint32(SCOPE.current_index);
                    SCOPE.current_index += Uint32Array.BYTES_PER_ELEMENT;
                    ...
                    // ignore default
                        ...
                        SCOPE.current_index = jmp_index; // jump to end of default block
                    }
                ...
                case BinaryCode.TIME: {
                    ...
                    let millis = SCOPE.buffer_views.data_view.getBigUint64(SCOPE.current_index, true);
                ...
                case BinaryCode.INIT_POINTER: {
                    ...
                    // pointer exists
                    try {
                        pointer = await Pointer.load(id, SCOPE);
                        SCOPE.current_index = jmp_index; // jump to end of init block
                    }
                    // does not exit - init
                    catch (e) {
