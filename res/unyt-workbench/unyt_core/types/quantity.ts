import { Unit, UnitCodeBySymbol, UnitCodeBySymbolShortForms, UnitPrefixCodeBySymbol, UnitSymbolShortFormsByCode, unit_factor, unit, unit_symbol, UnitSymbol, unit_prefix, code_to_extended_symbol, UnitAliases, DerivedUnits, symbol_to_code, code_to_symbol, unit_base_symbol } from "../compiler/unit_codes.js";
import { Type } from "./type.js";
import { RuntimeError, SyntaxError, ValueError } from "./errors.js";
...
// Quantity with unit
export class Quantity<U extends Unit = undefined> {
    ...
    constructor(value:number|bigint|string|[num:number|bigint, den:number|bigint] = 1, symbol_or_encoded_unit:string|unit = 'x') {
        ...
        // symbol string, parse
            ...
            let [factor_num, factor_den, unit] = this.parseUnit(symbol_or_encoded_unit);
            this.unit = unit;
    ...
    #raiseUnitToPower(unit:unit, power:bigint|number) {
        power = Number(power);
        const new_exp = [];
        for (let u of unit) new_exp.push([u[0], u[1] * power]);
        return new_exp;
    }
    ...
    hasBaseUnit<SYMBOL extends unit_base_symbol>(unit: Extract<SYMBOL, code_to_extended_symbol<U>>) { //: this is Quantity<symbol_to_code[SYMBOL]> {
    ...
    setUnitBinary() {
        ...
        for (let [code, exponent] of this.unit) {
            uint8[i++] = code;
        ...
        // get cached
        if (Quantity.cached_binaries.has(this.unit_formatted)) this.unit_binary = Quantity.cached_binaries.get(this.unit_formatted)
        // save in cache
        else {
    ...
    parseUnit(unit?:string):[factor_num:bigint, factor_den:bigint, unit:unit] {
        ...
        for (let part of parts) {
            ...
            let [symbol, exponent_string] = <[unit_symbol, string]>part.split("^");
            ...
            let expanded_symbol = this.getExpandedSymbolWithPrefix(symbol);

            for (let [factor_num, factor_den, unit, _exp] of expanded_symbol) {
                let exponent = (_exp??1) * exp; // multiply implicit exponent from expansion with exponent from unit string
    ...
    getExpandedSymbolWithPrefix(prefixed_symbol:string): expanded_symbol {
        let expanded_symbol = this.getExpandedSymbol(<unit_symbol>prefixed_symbol);
        // no prefix
        if (expanded_symbol != null) return expanded_symbol;

        // prefix
        else {
            let prefix_exp = UnitPrefixCodeBySymbol[prefixed_symbol[0]];
            // no valid prefix found
            if (prefix_exp == null) throw new SyntaxError("Invalid unit symbol '" + prefixed_symbol + "'")
           
            let expanded_symbol = this.getExpandedSymbol(<unit_symbol>prefixed_symbol.slice(1));
    ...
    getExpandedSymbol(symbol:unit_symbol): expanded_symbol {
    ...
    getCodeSymbol(symbol:Unit|unit_factor, abs_exponent = false){
        let exponent = abs_exponent ? Math.abs(symbol[1]) : symbol[1];
    ...
    setFormattedUnit(){
        ...
        for (let [expression, alias, divisor] of Quantity.known_aliases) {
    ...
    #combineEncodedUnits(unit1:unit, unit2:unit) {
        ...
        for (let part of unit2) this.#addUnitAndExponent(new_unit, ...part);
    ...
    #finiteFractionToDecimalString(numerator:bigint, denominator:bigint) {
        ...
        if (!Quantity.#isPowerOf10(denominator)) {
            ...
            for (let x=0;x<10000;x++) { // only try 10000 iterations, works in first iteration in most cases
                ...
                let new_denominator = 10n ** BigInt(shift) // new possible base 10 denominator
        ...
        let string = (numerator*BigInt(this.sign)).toString().padStart(shift,'0');
        let comma_shift = string.length-shift;
        let p1 = string.slice(0,comma_shift);
        let p2 = string.slice(comma_shift)
