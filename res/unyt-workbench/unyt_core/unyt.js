...
import Logger, { font_family } from "./logger.js";
...
export default class Unyt {
    static async login(endpoint, password) {
        if (!endpoint)
        ...
        try {
            ...
            let [sign_key, dec_key] = await this.decryptPrivateKeys(...res, password);
            let public_keys_base64 = await Datex.NetworkUtils.get_keys(endpoint);
    ...
    static async register(endpoint, password) {
        if (!endpoint)
            throw new Error("no endpoint");
        if (!password)
            throw new Error("no password");
        ...
        try {
            ...
            if (!endpoint.id_endpoint)
                endpoint.setIdEndpoint(current_endpoint);
            Datex.Runtime.endpoint = endpoint;
            await DatexCloud.sayHello();
    ...
    static async encryptPrivateKeys(sign_key, dec_key, password) {
    ...
    static async decryptPrivateKeys(sign_key_enc, dec_key_enc, iv_sign, iv_dec, password) {
        const key = await this.getKeyFromPassword(password);
    ...
    static logStatus(data) {
        ...
        console.log(`%cunyt %c${data.version || ""}\n` +
            ...
            `%c© ${new Date().getFullYear()} unyt.org`, `font-size:50px;color:#fff;font-weight:bold;padding-bottom:15px;`, `color:#aaa;font-size:18px;margin-left:-115px;${font_family};margin-top:20px`, `color:#aaa;${font_family};margin-left:115px;`, `font-weight:bold;color:#1eda6d; padding-top:10px;${font_family}`, `color:#aaa;${font_family};`, `color:#1eda6d;margin-left:0px; padding-top:10px;${font_family}`, `color:#aaa;padding:0px;${font_family};margin-left:0px;`, `color:#1eda6d; padding-top:3px;${font_family}`, `color:#aaa;padding:0px;${font_family}`, `color:#1eda6d;padding-top:3px;${font_family}`, `color:#aaa;padding:0px;${font_family}`, `color:#1eda6d;padding-top:3px;${font_family}`, `color:#aaa;padding:0px;${font_family}`, `color:inherit;${font_family}`, `color:#0669c1;${font_family};margin-left:20px;padding-top:10px;padding-bottom:5px;${font_family}`, `color:#aaa;${font_family};padding-top:10px;margin-bottom:30px;${font_family}`);
    }
}
let Auth = class Auth {
    static get_private_keys(endpoint, hash) { return null; }
    static set_private_keys(endpoint, hash, sign_key, dec_key, iv_sign, iv_dec) { return null; }
};
__decorate([
    remote,
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Datex.Addresses.Endpoint, ArrayBuffer]),
    __metadata("design:returntype", Promise)
], Auth, "get_private_keys", null);
__decorate([
    remote,
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Datex.Addresses.Endpoint, ArrayBuffer, ArrayBuffer, ArrayBuffer, ArrayBuffer, ArrayBuffer]),
    __metadata("design:returntype", Promise)
], Auth, "set_private_keys", null);
Auth = __decorate([
    root_extension,
    to('@+unyt:auth')
], Auth);
