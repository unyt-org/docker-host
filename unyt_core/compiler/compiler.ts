...
import { ReadableStream, Runtime} from "../runtime/runtime.js";
import { Endpoint, IdEndpoint, Target, WildcardTarget, Institution, Person, Bot, BROADCAST, target_clause, endpoints } from "../types/addressing.js";
import { Pointer, PointerProperty, Value } from "../runtime/pointers.js";
import { CompilerError, RuntimeError, Error as DatexError, ValueError } from "../types/errors.js";
...
import { ProtocolDataType } from "./protocol_types.js";
import { Quantity } from "../types/quantity.js";
import { EXTENDED_OBJECTS, INHERITED_PROPERTIES, VOID } from "../runtime/constants.js";
import { arrayBufferToBase64, base64ToArrayBuffer, buffer2hex, hex2buffer } from "../utils/utils.js";
import { RuntimePerformance } from "../runtime/performance_measure.js";
import { Conjunction, Disjunction, Logical, Negation } from "../types/logic.js";
import { Regex } from "./tokens_regex.js";
import { baseURL, client_type, TypedArray } from "../utils/global_values.js";
import { datex_scope } from "../utils/global_types.js";
import { NumberLiteralType } from "../../../node_modules/typescript/lib/typescript.js";
import { unit_symbol } from "./unit_codes.js";
import { Time } from "../types/time.js";
import { f } from "../datex_short.js";

// wasm compiler
let wasm_compile:Function
if (client_type == "browser") {
    const datex_wasm = await import("../wasm/datex/pkg/datex.js");
    await datex_wasm.default(); // init
    datex_wasm.init_runtime();
    wasm_compile = datex_wasm.compile;
}
...
export type compiler_options = {
    ...
    insert_header?: {
        ...
        vars: {[name:string]: [type:'val'|'var'|'ref', slot:number]} // reference to the vars of the parent insert scope
    }
...
export class Compiler {
    ...
    public static extractHeaderSender(dx_block: ArrayBuffer, last_byte?:[number], _appspace_byte = true, _start = 8): Endpoint {
    ...
    protected static extractHeaderReceiverDataList(dx_block: ArrayBuffer, start_byte:number):Map<Endpoint, ArrayBuffer> {
        ...
        let targets_map = new Map<Endpoint, ArrayBuffer>();
    ...
    public static updateHeaderReceiver(dx_block:ArrayBuffer, to:Disjunction<Endpoint>):ArrayBuffer {
        ...
        let sender = Compiler.extractHeaderSender(dx_block, last_byte);
        //console.log("> sender", sender, last_byte[0]);
        let keys = Compiler.extractHeaderReceiverDataList(dx_block, last_byte[0]+2);

        // now add the required keys back into the old header
        let receiver_buffer = Compiler.targetsToDXB(to, keys, true);
    ...
    public static async generateScopeBlockMetaInfo(
        ...
        if (!flood && to) {
            ...
            // encrypted keys map
            let endpoint_key_map = new Map<Endpoint, ArrayBuffer>();
            ...
            // endpoint or list of endpoints
                ...
                if (send_sym_encrypt_key && sym_encrypt_key) {
                    for (let endpoint of evaluated_endpoints) {
    ...
    public static endpointToDXB(target:Endpoint) {
        ...
        let name_bin = (target instanceof IdEndpoint) ? target.binary : this.utf8_encoder.encode(target.name); 
    ...
    public static targetsToDXB(clause:Disjunction<Endpoint>|Pointer<target_clause>, keys_map?:Map<Endpoint,ArrayBuffer>, extended_keys = false): ArrayBuffer {
        ...
        if (clause instanceof Pointer) {
            let buffer = new ArrayBuffer(1 + Pointer.MAX_POINTER_ID_SIZE) // estimated size
    ...
    static builder = {
        ...
        resolveValVarRef: async (SCOPE:compiler_scope, name:string): Promise<[type:'val'|'var'|'ref', index:number, parent_var:boolean]> => {
        ...
        insertVariable: (SCOPE:compiler_scope|extract_var_scope, v?:string|number, action_type:ACTION_TYPE = ACTION_TYPE.GET, action_specifier?: BinaryCode, base_type:BinaryCode=BinaryCode.INTERNAL_VAR,  index:number=SCOPE.b_index, init_brackets = false, prefix_action?: BinaryCode):void|Promise<void> => {
            ...
            if (action_type == ACTION_TYPE.INIT) return Compiler.builder.addInitBlock(<compiler_scope>SCOPE, init_brackets, prefix_action); // might be a promise
        ...
        addRelativePath: (path:string, SCOPE:compiler_scope) => {
            Compiler.builder.valueIndex(SCOPE);
            let str_bin = Compiler.utf8_encoder.encode(path);  // convert string to binary
            Compiler.builder.handleRequiredBufferSize(SCOPE.b_index+str_bin.byteLength+(Uint32Array.BYTES_PER_ELEMENT)+1, SCOPE);
        ...
        addQuantity: (u:Quantity, SCOPE:compiler_scope) => {
            Compiler.builder.handleRequiredBufferSize(SCOPE.b_index+Float64Array.BYTES_PER_ELEMENT+1+u.unit_binary.byteLength, SCOPE);
            Compiler.builder.valueIndex(SCOPE);
        ...
        addTime: (t:Time, SCOPE:compiler_scope) => {
            ...
            SCOPE.data_view.setBigUint64(SCOPE.b_index, BigInt(t.getTime()), true)
        ...
        addScopeBlock: async (type:BinaryCode, brackets:boolean, extract_pointers: boolean, SCOPE:compiler_scope) => {
            ...
            let compiled = <ArrayBuffer> await this.compile(return_data, SCOPE.data, {parent_scope:SCOPE}, false, true, extract_pointers, undefined, Infinity, brackets?1:2, SCOPE.current_data_index);
        ...
        addKey: (k:string|number|bigint, SCOPE:compiler_scope) => {
            ...
            // string key
            if (typeof k == "string") {
                let key_bin = Compiler.utf8_encoder.encode(k);  // convert key to binary
                Compiler.builder.handleRequiredBufferSize(SCOPE.b_index+key_bin.byteLength+1, SCOPE);
        ...
        addFilterTargetFromParts: (type:BinaryCode, name:string|Uint8Array, subspaces:string[], instance:string, appspace:Endpoint, SCOPE:compiler_scope) => {
            ...
            let name_bin = (name instanceof Uint8Array) ? name : Compiler.utf8_encoder.encode(name); 
            let instance_bin = Compiler.utf8_encoder.encode(instance); 
        ...
        addPersonByNameAndChannel: (name:string, subspaces:string[], instance:string, appspace:Endpoint, SCOPE:compiler_scope) => {
            Compiler.builder.addFilterTargetFromParts(BinaryCode.PERSON_ALIAS, name, subspaces, instance, appspace, SCOPE);
        },
      
        addBotByNameAndChannel: (name:string, subspaces:string[], instance:string, appspace:Endpoint, SCOPE:compiler_scope) => {
            Compiler.builder.addFilterTargetFromParts(BinaryCode.BOT, name, subspaces, instance, appspace, SCOPE);
        },

    
        addInstitutionByNameAndChannel: (name:string, subspaces:string[], instance:string, appspace:Endpoint, SCOPE:compiler_scope) => {
            Compiler.builder.addFilterTargetFromParts(BinaryCode.INSTITUTION_ALIAS, name, subspaces, instance, appspace, SCOPE);
        },
       
        addIdEndpointByIdAndChannel: (id:Uint8Array, subspaces:string[], instance:string, appspace:Endpoint, SCOPE:compiler_scope) => {
        ...
        addTypeByNamespaceAndName: (SCOPE:compiler_scope, namespace:string, name:string, variation?:string, parameters?:any[]|true) => {
            ...
            let name_bin = Compiler.utf8_encoder.encode(name);  // convert type name to binary
            let ns_bin = Compiler.utf8_encoder.encode(namespace);  // convert type namespace to binary
            let variation_bin = variation && Compiler.utf8_encoder.encode(variation);  // convert type namespace to binary

            Compiler.builder.handleRequiredBufferSize(SCOPE.b_index+name_bin.byteLength+ns_bin.byteLength+2+(is_extended_type?2:0), SCOPE);
        ...
        addPointerBodyByID: (id:string|Uint8Array, SCOPE:compiler_scope|extract_var_scope) => {
            let id_bin = id instanceof Uint8Array ? id : hex2buffer(id, Pointer.MAX_POINTER_ID_SIZE, true);  // convert pointer name to binary
            ...
            Compiler.builder.handleRequiredBufferSize(SCOPE.b_index+Pointer.MAX_POINTER_ID_SIZE, SCOPE);
        ...
        addInitBlock: async (SCOPE:compiler_scope, brackets = false, prefix_action?:BinaryCode) => {


            let return_data:{datex:string} = {datex: SCOPE.datex}; 
            let compiled = <ArrayBuffer> await this.compile(return_data, SCOPE.data, {parent_scope:SCOPE, pseudo_parent:true}, false, false, false, undefined, Infinity, brackets?1:2, SCOPE.current_data_index);
            SCOPE.datex = return_data.datex; // update position in current datex script
            ...
            // set jmp index
            SCOPE.data_view.setUint32(SCOPE.b_index, SCOPE.b_index + Uint32Array.BYTES_PER_ELEMENT +  compiled.byteLength + (prefix_action!=undefined ? 1 : 0));
            SCOPE.b_index += Uint32Array.BYTES_PER_ELEMENT;
            if (prefix_action!=undefined) SCOPE.uint8[SCOPE.b_index++] = prefix_action;
            SCOPE.uint8.set(new Uint8Array(compiled), SCOPE.b_index)
            SCOPE.b_index += compiled.byteLength;
        },
        ...
        addValVarRefDeclaration: async (name:string, type:'val'|'var'|'ref', SCOPE:compiler_scope, init = false, init_brackets = false) => {
            ...
            if (INNER_SCOPE.vars.hasOwnProperty(name)) throw new CompilerError("Cannot redeclare "+type+" '"+name + "'", SCOPE.stack);
                ...
                if (!SCOPE.var_index) SCOPE.var_index = 0x0100; // 0x00 - 0xff reserved for extracted variables for function, do, ...
        ...
        addPointerByID: (SCOPE:compiler_scope|extract_var_scope, id:string|Uint8Array, action_type:ACTION_TYPE = ACTION_TYPE.GET, action_specifier?:BinaryCode, init_brackets = false):Promise<void>|void => {
            ...
            if (action_type == ACTION_TYPE.INIT) return Compiler.builder.addInitBlock(<compiler_scope>SCOPE, init_brackets)
        },

        addPointer: (p:Pointer, SCOPE:compiler_scope, action_type:ACTION_TYPE = ACTION_TYPE.GET, action_specifier?:BinaryCode):Promise<void>|void => {

            // pre extract per default
            if (SCOPE.extract_pointers && action_type == ACTION_TYPE.GET) {
                Compiler.builder.insertExtractedVariable(SCOPE, BinaryCode.POINTER, buffer2hex(p.id_buffer));
            }
            // add normally
        ...
        addArray: (a:Array<any>, SCOPE:compiler_scope, is_root=true, parents:Set<any>=new Set(), unassigned_children:[number, any, number][]=[], start_index:[number]=[0]) => {
            ...
            let trimmed_length = Runtime.runtime_actions.getTrimmedArrayLength(a);
            ...
            for (let i = 0; i<trimmed_length; i++) {
                ...
                if (SCOPE.inserted_values.has(val) && parents.has(val)) {
                    ...
                    let existing_val_var = val == a ? parent_var : Compiler.builder.createInternalVariableAtIndex(value_index, SCOPE, val)
        ...
        insert: (value:any, SCOPE:compiler_scope, is_root=true, parents?:Set<any>, unassigned_children?:[number, any, number][], add_insert_index = true) => {
            ...
            if (add_insert_index && SCOPE.inserted_values?.has(value)) {
                ...
                let existing_val_var = Compiler.builder.createInternalVariableAtIndex(value_index, SCOPE, value)
            ...
            let start_index = Compiler.builder.getDynamicIndex(SCOPE.b_index, SCOPE);
            ...
            if (value instanceof Date && !(value instanceof Time)) value = new Time(value);
            ...
            const no_proxify = value instanceof Value && (((value instanceof Pointer && value.is_anonymous) || SCOPE.options.collapse_pointers) || SCOPE.options.collapse_first_inserted);

            // proxify pointer exceptions:
            if (no_proxify) {

                // handle pointers with transform (always ...)
                if (value instanceof Pointer && value.transform_scope) {
                    if (SCOPE.options.collapse_first_inserted) SCOPE.options.collapse_first_inserted = false; // unset before calling insert() again

                    const compiled = value.transform_scope.compiled;

                    SCOPE.uint8[SCOPE.b_index++] = BinaryCode.TRANSFORM;

                    for (let v of value.transform_scope.internal_vars) {
                ...
                // add $$ operator, not if no_create_pointers enabled or collapse_first_inserted
                if (SCOPE.options.collapse_pointers && !(SCOPE.options.no_create_pointers||SCOPE.options.collapse_first_inserted)) SCOPE.uint8[SCOPE.b_index++] = BinaryCode.CREATE_POINTER;
            }
            ...
            if (value instanceof Quantity)                   Compiler.builder.addQuantity(value, SCOPE); // UNIT
                ...
                // $pointer
                Compiler.builder.addPointer(value.pointer, SCOPE);
                // ->
                let _SCOPE = SCOPE.extract_pointers ? SCOPE.extract_var_scope : SCOPE;
                ...
                // injected vars
                // don't collapse injected var pointers
                let _collapse_pointers = SCOPE.options.collapse_pointers;
                SCOPE.options.collapse_pointers = false;
                for (let v of value.internal_vars) {
                    Compiler.builder.insert(v, SCOPE);
                }
    ...
    static async parseNextExpression (SCOPE:compiler_scope) {
        ...
        // INSERT data (?)
            ...
            if (SCOPE.precompiled) {
                ...
                Compiler.builder.insert(d, SCOPE);
            }
            isEffectiveValue = true;
        }

        // use compile get instead
         
        // INSERT inline command
        // value resolution similar to 'get'
        // resolve URL value
        // resolve endpoint #default
        // insert scope as script code
            ...
            let cache = SCOPE.options.insert_header.cache;
            ...
            try {
                ...
                // already in insert cache
                if (cache.has(value.toString())) {
                    cache_var = cache.get(value.toString())
                    console.log("insert CACHED", value.toString(), cache_var);
                ...
                // create new  
                    ...
                    let var_index = SCOPE.var_index++;
                    cache.set(value.toString(), cache_var);
                    SCOPE.options.insert_header.vars[cache_var] = ["var", var_index]; // points to the SCOPE.inner_scope.vars of the parent scope
    
                    // get script/resource from url
                    if (value instanceof URL)  {
                        const [data,type] = await Runtime.getURLContent(value, true)
                        console.log("url resource", type)
    
                        // DATEX Script file
                        if (type.startsWith("text/datex") || value.toString().endsWith(".dx")) compiled_script = <ArrayBuffer> await this.compile(data, [], {context_location:value, insert_header:SCOPE.options.insert_header, parent_scope:SCOPE, only_leak_inserts:true}, false, true);
                        // JSON file
                        else if (type.startsWith("application/json")) compiled_script = <ArrayBuffer> await this.compile(data, [], {context_location:value, insert_header:SCOPE.options.insert_header, parent_scope:SCOPE, only_leak_inserts:true}, false, true);
                        // TODO dxb
                        //else if (type == "application/datex") compiled_script = data;
    
                        else throw new CompilerError("Unsupported compile resource type: '" + type + "'", SCOPE.stack);
                    ...
                    if (compiled_script) {
                        ...
                        let insert_buffer = SCOPE.options.insert_header.buffer;
                        ...
                        function handleRequiredBufferSize(size_in_bytes:number) {
                            ...
                            if (size_in_bytes>=insert_buffer.byteLength-1) {
                                let new_size = (insert_buffer?.byteLength??0)+add_bytes;
                                //logger.info("extending buffer size to " + new_size + " bytes");
                                let old_uint8 = insert_uint8;
                                SCOPE.options.insert_header.buffer = insert_buffer = new ArrayBuffer(new_size);
                        ...
                        handleRequiredBufferSize(SCOPE.options.insert_header.index+20+compiled_script.byteLength);

                        // #x =
      
                        insert_uint8[SCOPE.options.insert_header.index++] = BinaryCode.SET_INTERNAL_VAR;
                        insert_uint8[SCOPE.options.insert_header.index++] = 0;
                        insert_dataview.setUint16(SCOPE.options.insert_header.index, var_index, true);
                        SCOPE.options.insert_header.index += Uint16Array.BYTES_PER_ELEMENT;

                        // add do
                        insert_uint8[SCOPE.options.insert_header.index++] = BinaryCode.DO;

                        insert_uint8.set(new Uint8Array(compiled_script), SCOPE.options.insert_header.index)
                        SCOPE.options.insert_header.index += compiled_script.byteLength;
                        insert_uint8[SCOPE.options.insert_header.index++] = BinaryCode.CLOSE_AND_STORE;
                    }


                    // resolve from cache_var name
                    const [type, index, parent_var] = await Compiler.builder.resolveValVarRef(SCOPE, cache_var);
                    // variable not found
                    if (index == -1) throw new CompilerError("Variable '"+name+"' was not declared in scope", SCOPE.stack);
        ...
        // KEY  (check before variable and keywords!, only if not in :: filter)
            ...
            const permission_prefix = Compiler.builder.check_perm_prefix(SCOPE);
            ...
            Compiler.builder.addKey(int_key??key, SCOPE);
            isEffectiveValue = true;
        }
        ...
        // PATH_REF_SEPERATOR (->)
            ...
            let _SCOPE = (SCOPE.extract_pointers && !wildcard) ? SCOPE.extract_var_scope : SCOPE;
        ...
        // TEMPLATE STRING (before OBJECT_END) '... (
            ...
            let str = Compiler.builder.unescape_string(escaped_string);
        ...
        for (let i=0;i<500_000;i++) {            
            ...
            if (SCOPE.end || !SCOPE.datex) { 
                ...
                for (let scope of SCOPE.subscopes) {
                    if (scope.parent_type == BinaryCode.OBJECT_START) throw new SyntaxError("Missing closing object bracket");
    ...
    static valueToBase64DXB(value:any, inserted_ptrs?:Set<Pointer>):string {
        let dxb = Compiler.compileValue(value, {inserted_ptrs})
    ...
    static async datexScriptToBase64DXB(dx:string, type = ProtocolDataType.DATA, data = []):Promise<string> {
        let dxb = <ArrayBuffer> await Compiler.compile(dx, data, {sign:false, encrypt: false, type})
        return arrayBufferToBase64(dxb);
    }

    /** create and download (if file_name not null) a dxb file created from a DATEX Script string */
    static async datexScriptToDXBFile(dx:string, file_name?:string, type = ProtocolDataType.DATA, data = [], collapse_pointers = false):Promise<Blob> {
        let dxb = <ArrayBuffer> await Compiler.compile(dx, data, {sign:false, encrypt: false, type, collapse_pointers})
    ...
    static encodeValue(value:any, inserted_ptrs?:Set<Pointer>, add_command_end = true, deep_clone = false, collapse_first_inserted = false, no_create_pointers = false):ArrayBuffer {
    ...
    static encodeValueBase64(value:any, inserted_ptrs?:Set<Pointer>, add_command_end = true, deep_clone = false, collapse_first_inserted = false, no_create_pointers = false):string {
    ...
    static async compilePrecompiled(precompiled:PrecompiledDXB, data:any[] = [], options:compiler_options={}, add_header=true):Promise<ArrayBuffer> {
        ...
        for (let part of precompiled) {
            if (part instanceof ArrayBuffer) buffer = part;
                ...
                // compile value
                else buffer = compiled_cache[data[part]] = await Compiler.compileValue(data[part], undefined, false);
            }
    ...
    static compile(datex:string|{datex:string}|PrecompiledDXB, data:any[] = [], options:compiler_options={}, add_header=true, is_child_scope_block = false, extract_pointers:boolean = false, save_precompiled?:PrecompiledDXB, max_block_size?:number, _code_block_type?:0|1|2|42, _current_data_index=0): Promise<ArrayBuffer|ReadableStream<ArrayBuffer>>|ArrayBuffer {
    ...
    static createCompilerScope(datex:string|{datex:string}, data:any[] = [], options:compiler_options={}, add_header=true, is_child_scope_block = false, extract_pointers:boolean = false, save_precompiled?:PrecompiledDXB, max_block_size?:number, _code_block_type?:0|1|2|42, _current_data_index=0):compiler_scope{
...
globalThis.DatexCompiler = Compiler;
