...
import { SecurityError, ValueError } from "../types/errors.js";
import { NetworkUtils } from "../network/network_utils.js";
...
// @ts-ignore
export const crypto = globalThis.crypto ?? <globalThis.Crypto>(<any>(await import("crypto")).webcrypto) //<- EXPERIMENTAL, deprecated ->  <Crypto>(<any>new((await import("node-webcrypto-ossl")).default.Crypto)()) 
...
export class Crypto {
    ...
    private static public_keys = new Map<Endpoint, [CryptoKey, CryptoKey]>(); // verify_key, enc_key
    ...
    static async getKeysForEndpoint(endpoint:Endpoint):Promise<[CryptoKey, CryptoKey]> {
    ...
    static async requestKeys(endpoint:Endpoint):Promise<[CryptoKey, CryptoKey]> {
