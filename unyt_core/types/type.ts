import { DX_PERMISSIONS, DX_TEMPLATE, DX_TYPE, EXTENDED_OBJECTS, INVALID, NOT_EXISTING, UNKNOWN_TYPE, VOID } from "../runtime/constants.js";
import { Runtime, TypedValue } from "../runtime/runtime.js";
import { JSInterface, js_interface_configuration } from "../runtime/js_interface.js";
import { Endpoint, Target } from "./addressing.js";
import { AssertionError, CompilerError, NetworkError, PermissionError, PointerError, RuntimeError, SecurityError, SyntaxError, TypeError, ValueError } from "./errors.js";
...
import { Function as DatexFunction } from "./function.js";
import { logger, NodeBuffer, TypedArray } from "../utils/global_values.js";
import { BinaryCode } from "../compiler/binary_codes.js"
import { CompatValue, Pointer, Value } from "../runtime/pointers.js";
import { clause, Conjunction, Disjunction, Logical, LogicalComparator, Negation } from "./logic.js";
...
export class Type<T = any> {
    ...
    public static ofValue<T=any>(value:CompatValue<T>):Type<T> {
        ...
        if (!custom_type) {
            ...
            if (value instanceof ArrayBuffer || value instanceof NodeBuffer || value instanceof TypedArray) return <Type<T>>Type.std.buffer;
            ...
            if (value instanceof Markdown) return <Type<T>>Type.std.Markdown;
    ...
    public static getClassDatexType<T=any>(forClass:{new():T}):Type<T> {
        ...
        if (!custom_type) {
            ...
            if (_forClass == ArrayBuffer || _forClass == NodeBuffer || TypedArray.isPrototypeOf(_forClass)) return <Type<T>>Type.std.buffer;
    ...
    static std = {
        ...
        text_plain: Type.get<string>("std:text").getVariation("plain"),
        text_datex: Type.get<string>("std:text").getVariation("datex"),
        text_markdown: Type.get<string>("std:text").getVariation("markdown"),

        image: Type.get<string>("std:image"),
        video: Type.get<string>("std:video"),
        audio: Type.get<string>("std:audio"),
        model: Type.get<string>("std:model"),

        application: Type.get<string>("std:application"),
