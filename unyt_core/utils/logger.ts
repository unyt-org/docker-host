...
import type { Pointer } from "../runtime/pointers.js";
...
const ESCAPE_SEQUENCES_CSS = {
    ...
    // foreground colors
    30: `color:black;`,
    31: `color:rgb(${UNYT_COLORS.RED.join(',')});`,
    32: `color:rgb(${UNYT_COLORS.GREEN.join(',')});`,
    33: `color:rgb(${UNYT_COLORS.YELLOW.join(',')});`,
    34: `color:rgb(${UNYT_COLORS.BLUE.join(',')});`,
    35: `color:rgb(${UNYT_COLORS.MAGENTA.join(',')});`,
    36: `color:rgb(${UNYT_COLORS.CYAN.join(',')});`,
    37: `color:white;`,

    // background colors
    40: `background-color:black;`,
    41: `background-color:rgb(${UNYT_COLORS.RED.join(',')});`,
    42: `background-color:rgb(${UNYT_COLORS.GREEN.join(',')});`,
    43: `background-color:rgb(${UNYT_COLORS.YELLOW.join(',')});`,
    44: `background-color:rgb(${UNYT_COLORS.BLUE.join(',')});`,
    45: `background-color:rgb(${UNYT_COLORS.MAGENTA.join(',')});`,
    46: `background-color:rgb(${UNYT_COLORS.CYAN.join(',')});`,
...
export let console_theme:"dark"|"light" = (client_type=="node" || globalThis.matchMedia && globalThis.matchMedia('(prefers-color-scheme: dark)')?.matches) ? "dark" : "light";
...
class DATEXFormatter implements LogFormatter {
    ...
    formatValue(value: any, params: any[]): string {
        return _Runtime.valueToDatexString(value, true, true);
    }
...
class ImageFormatter implements LogFormatter {
    ...
    formatValue(value: any, params: any[]): string {
        // %%img(height;;placeholder-text;;src)
        return  `%%img(${params[0]??''};;${params[1]??''};;${value})`
    }
...
export class Logger {
    ...
    private logRaw(text:string, log_level:LOG_LEVEL = LOG_LEVEL.DEFAULT, only_log_own_stream = false, browser_compat_mode_required = false) {
        ...
        if (this.log_to_console) {
            ...
            if (client_type == "browser") console_log(
                (Logger.browser_supports_ansi_escape_codes&&!browser_compat_mode_required) ? 
                    this.convertLogStringForBrowser(text): 
                    this.convertLogStringForBrowserCompat(text), log_level
            );
            else {
                // browser_compat_mode_required is forced, convert incompatible browser specific formatting (e.g. images)
                if (browser_compat_mode_required) text = Logger.convertCompatStringToEscapeSequences(text);
                console_log([text]);
            }
            // browser_compat_mode_required is forced, convert incompatible browser specific formatting (e.g. images)
            if (browser_compat_mode_required) text = Logger.convertCompatStringToEscapeSequences(text);
        ...
        // browser_compat_mode_required is forced, convert incompatible browser specific formatting (e.g. images)
        else if (browser_compat_mode_required) text = Logger.convertCompatStringToEscapeSequences(text);


        // handle log streams
        if (!only_log_own_stream) {
            for (let stream of Logger.global_log_streams) stream.write(text+"\r\n");
        }
    ...
    private static custom_formatting = /%%img\([^\)]*\)/g
    private static csi_color_escape_sequence = /(?:\x1b\[((?:\d{0,4};)*\d{0,4})?m|%%img\([^\)]*\))/g
    private static multiple_cs = /%c(%c)*/g
    private static inverse_underline_block = /\x1b\[7m\x1b\[4m(\x1b\[(?:(?:\d{0,4};)*\d{0,4})?m)/g

    // removes/modifies certain esacpe sequences
    private convertLogStringForBrowser(text:string){
        return [
            text
                // replace UNDERLINE+INVERSE, (\x1b[7m is currently not supported by chrome)
                .replace(Logger.inverse_underline_block, (_,c) => this.getFormattingColor(console_theme == 'dark' ?  COLOR.BLACK : COLOR.WHITE) + Logger.getEscapedBackgroundColor(c))
                .replaceAll(ESCAPE_SEQUENCES.CLEAR, '')
        ]
    }

    // for firefox & safari, they don't support escape sequences
    // adds %c console formatting

    private convertLogStringForBrowserCompat(text:string){
        const parts = this.convertLogStringForBrowser(text);
        
        let c_index = 1;
        
        // replace escape seq with %c
        parts[0] = parts[0].replace(Logger.csi_color_escape_sequence, (x,p)=> {
                const params:number[] = p?.split(";").map(Number)
                let new_style:string;

                // %%img
                if (!params && x.startsWith("%%img(")) {
                    const image = x.replace('%%img(', '').replace(/\)$/, '').split(";;");
                    const h = Number(image[0]);
                    const placeholder = image[1] || " "
                    const src = image[2];
                    new_style = `color:transparent;font-size:${h}px;background-repeat:no-repeat;background-size:100% ${h}px;background-image:url(${src});`

                    parts.push((parts.length > 1 ? parts[parts.length-1] : '') + (new_style??''));
                    return "%c"+placeholder;
                }

                // rgb foreground
                else if (params[0] == 38 && params[1] == 2) {
                    new_style = `color:rgb(${params[2]},${params[3]},${params[4]});`
                }
                // rgb background
                else if (params[0] == 48 && params[1] == 2) {
                    new_style = `background-color:rgb(${params[2]},${params[3]},${params[4]});`
                }
                ...
                // 4 bit colors
                else if (params.length == 1 && params[0] > 0 && params[0] < 48) {
                    new_style = ESCAPE_SEQUENCES_CSS[params[0]];
                }
                ...
                // append new style
                parts.push((parts.length > 1 ? parts[parts.length-1] : '') + (new_style??'')); // also copied to %%img above
                return "%c";
            });
    ...
    // converts imcompatible non-esq-sequence formatting (%%img) to escape sequence compatible string
    private static convertCompatStringToEscapeSequences(text:string){
        return text.replace(this.custom_formatting, (t)=>{
            // %%img
            if (t.startsWith("%%img(")) {
                const image = t.replace('%%img(', '').replace(/\)$/, '').split(";;");
                return image[1]  ? 
                    '\n\r' + ESCAPE_SEQUENCES.BOLD +  image[1] + ESCAPE_SEQUENCES.RESET + '\n\r': 
                    ESCAPE_SEQUENCES.RED +  "[untitled image]" + ESCAPE_SEQUENCES.RESET;
            }
        })
    }
    ...
    private generateTag(color:COLOR) {
        ...
        if (esc_tag) {
            tag += 
                ESCAPE_SEQUENCES.INVERSE+ESCAPE_SEQUENCES.UNDERLINE +
                color_esc +
    ...
    private generateLogString(color:COLOR, text:string, data:any[], add_tag = true, browser_compat_mode_required:[boolean]): string {
        ...
        let message = Logger.applyLogFormatters(Logger.formatEscapeSequences(text, color_esc), data, color_esc, browser_compat_mode_required, this.formatting)
            .replace(/\n/g, '\r\n');


        let log_data = this.formatting == LOG_FORMATTING.PLAINTEXT ? 
    ...
    private static applyLogFormatters(text:string, data:any[], main_color:string, browser_compat_mode_required:[boolean], formatting:LOG_FORMATTING){
        return text.replace(Logger.match_log_formatter, (all,name,_args,outer_content,content) => {
            ...
            // formatter command
                ...
                const formatter = this.formatters.get(name) ?? this.formatters.get('text'); // fallback formatter is 'text'
        ...
        .replaceAll("\\?","\?") // replace escaped question marks
    }
    ...
    private static parseInsertValueDefault(value:any, main_color:string) {
        ...
        else return this.formatters.get('datex').formatValue(value, [], main_color);
    }

    private static parseFormatterArgument(arg:string, vars:Object): any {
        // variable
        if (vars && arg in vars) return vars[arg];
        // strings (" or ')
    ...
    public box(title?:string, message?:string) {
        ...
        for (let m of message_array || []) {
            m = m.trim();
            let formatters_length = m.length - Logger.removeEscapeFormatters(m).length;
    ...
    public debug(text:TemplateStringsArray, ...data:any[])
    ...
    public info(text:TemplateStringsArray, ...data:any[])
    ...
    public warn(text:TemplateStringsArray, ...data:any[])
    ...
    public error(text:TemplateStringsArray, ...data:any[])
    ...
    public success(text:TemplateStringsArray, ...data:any[])
    ...
    public plain(text:TemplateStringsArray, ...data:any[])
    public plain(text:string, ...data:any[])
    public plain(value:any)
    ...
    public dynamic(text:TemplateStringsArray, ...data:any[])
    public dynamic(text:string, ...data:any[])
